  ...
  const ImagePick = ({onChoosing}) => {
    ...
    const pickImage = async () => {
      ...
      if (!result.cancelled) {
        ...
        source = { uri: result.uri.split("//")[1], fileName, type: `${result.uri.split(".")[1]}` };
        setImage(result.uri);
        onChoosing(source);
      }
    };
...
const RecorderScreen = ({ navigation }) => {
   ...
   const authToken = () => {
      AsyncStorage.getItem("id_token", (err, result) => {
        ...
        console.log(result)
          ...
          //    data: `RNFetchBlob-file://${image.uri}`
          //   },
  ...
  const postSound = () => {
    ...
    formdata.append("soundcard[image]", {uri: image.uri, name: `${image.fileName}`, type: `${image.type}`})
    formdata.append("soundcard[audiofile]", {uri: audiofile.uri, name: `${audiofile.fileName}`, type: `${audiofile.type}`})
  ...
  async function stopRecording() {
    ...
    const source = { uri: recording.getURI(), type: 'audio/m4a', fileName: `newShum${recording.getURI().split("/AV/")[1]}`}
    setAudiofile(source);
